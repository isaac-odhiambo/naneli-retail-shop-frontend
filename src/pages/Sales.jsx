            ...
            <td className="px-4 py-2">
              {sale.items?.map((item) => (
                <div key={`${item.product_id}-${item.product_name}`}>
                  {item.product_name} x{item.quantity} - ${item.price ? item.price.toFixed(2) : 'N/A'}
                </div>
              ))}
            </td>
            <td className="px-4 py-2">${sale.total ? sale.total.toFixed(2) : 'N/A'}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <div className="h-[calc(100vh-6rem)] p-6">
      ...
      <div className="mt-6">
        ...
        <div className="space-y-2">
          <p>
            <strong>Total Sales for Selected Period:</strong> ${getTotalSales()}
          </p>
          <p>
            <strong>Total Profit for Selected Period:</strong> ${getProfit()}
          </p>
        </div>
      </div>
    </div>
...
//   const salesForThisWeek = useSelector(selectSalesForThisWeek);  // This week's sales
//   const salesForThisMonth = useSelector(selectSalesForThisMonth);  // This month's sales
//   const profitForToday = useSelector(selectProfitForToday);  // Today's profit
//   const profitForThisWeek = useSelector(selectProfitForThisWeek);  // This week's profit
//   const profitForThisMonth = useSelector(selectProfitForThisMonth);  // This month's profit
//   const profitOverall = useSelector(selectProfitOverall);  // Overall profit
//   const loading = useSelector(selectSalesLoading); // Get loading state
...
//             <td className="px-4 py-2">
//               {sale.items?.map((item) => (
//                 <div key={`${item.product_id}-${item.product_name}`}> {/* Ensured the key is unique by combining product_id and product_name */}
//                   {item.product_name} x{item.quantity} - ${item.price ? item.price.toFixed(2) : 'N/A'}
//                 </div>
//               ))}
//             </td>
...
//   // Get the profit for the selected period
//   const getProfit = () => {
//     switch (view) {
//       case 'daily':
